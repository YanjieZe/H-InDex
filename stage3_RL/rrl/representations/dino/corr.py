...
class PretrainedCorrespondence(nn.Module):
    ...
    def __init__(self, img_size, pretrain_k=200, tau_img=1.0, tau_mesh=1.0, 
                    corr_h=56, corr_w=56,
                    mesh=None, dino_path=None):
        super().__init__()
        self.mesh = mesh
        self.net = DINO(dino_path).eval()  # the pretrained net
        ...
        # use pixel coordinates as the meshgrid
        self.meshgrid_pixel = torch.Tensor(np.array(np.meshgrid(range(self.img_size), range(self.img_size)))).cuda().reshape(2, -1) - 0.5# 2,h*w

        for param in self.net.parameters():
            param.requires_grad = False


    def get_mask(self, img, threshold=1.):
        # use dino attention to get mask
        bsz, C, H, W = img.shape
        with torch.no_grad():
            torch.cuda.empty_cache()
            attention = self.net(img, return_cls_attention=True)[1]
        # interpolate to the original size
        attention = F.interpolate(attention, (H, W), mode='bilinear', align_corners=False)
        attention = attention.mean(dim=1)
        
        # compute auto threshold
        threshold = attention.mean() + threshold * attention.std()
        # get masking
        mask = (attention > threshold).float()
        return mask


    def match(self, src_img, tgt_img, src_mask, tgt_mask, grid=None):
        # return the match
        bsz = src_img.shape[0]
        ...
        # grid_match = torch.gather(grid, -1, indices_match[:, None].repeat(1,2,1))  # b,2,k
