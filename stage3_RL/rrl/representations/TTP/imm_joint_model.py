...
class HumanIMMModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pose_net.conv1(x) # [b, 64, 112, 112]
        x = self.pose_net.bn1(x)
        x = self.pose_net.relu(x)
        x = self.pose_net.maxpool(x) # [b, 64, 56, 56]

        x = self.pose_net.layer1(x) # [b, 256, 56, 56]
        x = self.pose_net.layer2(x) # [b, 512, 28, 28]
        x = self.pose_net.layer3(x) # [b, 1024, 14, 14]
        x = self.pose_net.layer4(x) # [b, 2048, 7, 7]
