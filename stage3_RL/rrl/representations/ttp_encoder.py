...
class TTPEncoder(nn.Module):

    def __init__(self, cfg_path, ckpt_path):
        super(TTPEncoder, self).__init__()
...
class TTPEncoderBN(nn.Module):

    def __init__(self, cfg_path, ckpt_path, bn_momentum=0.01):
        super(TTPEncoderBN, self).__init__()
        cfg = omegaconf.OmegaConf.load(cfg_path)
        ...
        print(colored("[TTPEncoderBN] Encoder loaded from {}".format(ckpt_path), "cyan"))
...
class TTPHumanEncoder(nn.Module):

    def __init__(self, cfg_path, ckpt_path):
        super(TTPHumanEncoder, self).__init__()
        cfg = omegaconf.OmegaConf.load(cfg_path)
        self.pose_net = get_pose_net(cfg, is_train=False, is_finetune=False, freeze_bn=False)
        ckpt = torch.load(ckpt_path)
        self.pose_net.load_state_dict(ckpt, strict=True)

        print(colored("[TTPHumanEncoder] Encoder loaded from {}".format(ckpt_path), "cyan"))
