...
def train_agent(job_name, agent,
    ...
    for i in range(i_start, niter):
        ...
        if evaluation_rollouts is not None and evaluation_rollouts > 0:
            ...
            mean_SR = np.mean([ path['is_success'] for path in eval_paths])
            if agent.save_logs:
                agent.logger.log_kv('eval_score', mean_pol_perf)
                agent.logger.log_kv('eval_success', mean_SR)
                eval_dict = {'eval_score': mean_pol_perf, 'eval_success_rate': mean_SR}
                agent.logger.log_dict(eval_dict, step=agent.frame_count)

                # save video
                obs_video = eval_paths[0]['observations_img'].cuda()
                obs_video = obs_video[:, 0] # select first camera (torch.tensor, t, c, h, w)

                if obs_video.max() > 100.0:
                    # no need to do imgnet norm reverse
                    pass
                elif obs_video.min() < 0.0 or obs_video.max() > 1.0:
                    # do imgnet norm reverse
                    obs_video = obs_video * torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1).cuda()
                    obs_video = obs_video + torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1).cuda()
                    # to uint8
                    obs_video = obs_video * 255.0
                    obs_video = obs_video.type(torch.uint8) # convert to uint8
                else:
                    # to uint8
                    obs_video = obs_video * 255.0
                    obs_video = obs_video.type(torch.uint8) # convert to uint8
                ...
                # to numpy
                obs_video = obs_video.cpu().numpy()

                agent.logger.log_video('eval_video', obs_video, step=agent.frame_count)

                cprint(f"[wandb] eval: frame: {agent.frame_count}, score: {mean_pol_perf:.3f}, success_rate: {mean_SR:.3f}", 'cyan')
            
            # if use_wandb:
            #     wandb.log({'eval/return': mean_pol_perf, 'eval/success_rate': mean_SR})
        ...
        if i % save_freq == 0:
            ...
            # log in wandb
            agent.logger.log_ckpt('iterations/best_policy.pickle', step=agent.frame_count)
