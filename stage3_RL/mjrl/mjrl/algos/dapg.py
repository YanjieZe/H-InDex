...
class DAPG(NPG):
    def __init__(self, env, policy, baseline,
        ...
        self.iter_count = 0.0
        self.frame_count = 0
        if save_logs: self.logger = DataLog(use_wandb=use_wandb)
    ...
    def train_from_paths(self, paths):
        ...
        self.frame_count += sum([path["observations"].shape[0] for path in paths])
        ...
        # Log information
        if self.save_logs:
            step = int(self.frame_count)
            self.logger.log_kv('alpha', alpha, step=step)
            self.logger.log_kv('delta', n_step_size, step=step)
            self.logger.log_kv('time_vpg', t_gLL, step=step)
            self.logger.log_kv('time_npg', t_FIM, step=step)
            self.logger.log_kv('kl_dist', kl_dist, step=step)
            self.logger.log_kv('surr_improvement', surr_after - surr_before, step=step)
            self.logger.log_kv('running_score', self.running_score, step=step)

            success_rate = np.mean([ path['is_success'] for path in paths])

            # try:
            #     # adroit env
            #     success_rate = self.env.env.env.evaluate_success(paths)
            #     self.logger.log_kv('success_rate', success_rate, step=step)
            # except:
            #     try:
            #         # dexmv env
            #         success_rate = self.env.env.evaluate_success(paths)
            #     except:
            #         raise Exception("Cannot evaluate success rate.")
            ...
            self.logger.log_dict(dict_to_log, step=step)
            cprint(f"[wandb] train: frame: {step}, score: {self.running_score:.2f}, success_rate: {success_rate:.2f}", 'cyan')
        return base_stats
