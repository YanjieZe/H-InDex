# bash scripts/adroit/train.sh pen ttp_frankmocap_hand_bn0.001 withSR 0 0
# bash scripts/adroit/train.sh pen vc1 withSR 0 0
# bash scripts/adroit/train.sh relocate resnet50 dense 0 2

num_cpu=4
use_wandb=0

# available tasks: hammer, door, pen, relocate
task=${1}
# available encoder type: 
#   resnet50/resnet34, rrl, dino, r3m, pvr, mvp, 
#   frankmocap_hand, frankmocap_body, alphapose,
#   frankmocap_hand_bn, resnet50_bn
...
if [[ $encoder_type == "frankmocap_hand" ]]; then
  ...
  encoder_ckpt="${cur_dir}/archive/adapted_frankmocap_hand_ckpts/${task}-v0.pth"
...
CUDA_VISIBLE_DEVICES=${gpu} python train_adroit.py \
            use_wandb=${use_wandb} \
