...
class PoseDoubleHeadResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if VIS_FEAT_MAP:
            ...
            feat = x[0].mean(0).detach().cpu().numpy()
            plt.imshow(feat)
            # remove all axes, ticks, and blank space
            plt.axis('off')
            plt.savefig('feat.png', bbox_inches='tight', pad_inches=0)
            import ipdb; ipdb.set_trace()


        x = self.deconv_layers(x) # [b, 256, 56, 56]
