...
class MugPlaceObjectEnv(MujocoEnv):
    def __init__(self, has_renderer, render_gpu_device_id=-1, object_scale=1, randomness_scale=1, mug_scale=2,
                 large_force=True, load_eval_mesh=False, arena_id=0, **kwargs):
    ...
    def reward(self, action):
        ...
        self.obj_target_distance_xy = obj_target_distance_xy
    ...
    def compute_success(self):
        "distance to the target object < 0.05"
        threshold = 0.4
        success = 1.0 if self.obj_target_distance_xy < threshold else 0.
        return success, self.obj_target_distance_xy, success
