...
class YCBRelocate(MujocoEnv):
    def __init__(self, has_renderer, render_gpu_device_id=0, object_name="mug", object_scale=1, randomness_scale=1,
                 use_visual_obs=False,  arena_id=0,
                 **kwargs):
    ...
    def _reset_internal(self):
        ...
        """
        high randomization of target object position
        """
        ...
        """
        low randomization of target object position
        """
        # self.mjpy_model.body_pos[self.target_object_bid, 0] = self.np_random.uniform(low=-0.10, high=0.10)
        # self.mjpy_model.body_pos[self.target_object_bid, 1] = self.np_random.uniform(low=-0.15, high=0.15)
        # self.mjpy_model.body_pos[self.target_object_bid, 2] = self.np_random.uniform(low=0.19, high=0.21)
    ...
    def compute_success(self):
        ...
        obj_target_distance = np.linalg.norm(obj_pos - target_pos)
        threshold = 0.1
        success = 1.0 if obj_target_distance < threshold else 0.0
        return success, obj_target_distance, success
    ...
    def _load_model(self):
        arena = TableArena(table_full_size=(1.2, 1.2, 0.05), table_friction=(1, 0.5, 0.01), table_offset=(0, 0, 1.0),
                           bottom_pos=(0, 0, -1), has_legs=True, xml="arenas/table_arena.xml" if self.arena_id==0 else f"arenas/table_arena_{self.arena_id}.xml")
        ...
        alpha = 0.125 if self.object_name not in ALPHA_DICT.keys() else ALPHA_DICT[self.object_name]
        target_geom = ET.Element("geom", type="mesh", mesh=f"{self.object_name}_visual_mesh", contype="0",
                                 conaffinity="0", rgba=f"0 1 0 {alpha}")
