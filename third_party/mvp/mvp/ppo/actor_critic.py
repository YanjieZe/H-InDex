...
import mvp.ppo.representations as representations
...
_MODELS = {
    ...
    # ours
    "mvp": "mae_pretrain_hoi_vit_small.pth",
    "frankmocap_hand": "frankmocap_hand.pth",
    "r3m": "r3m",
}
_MODEL_FUNCS = {
    "vits": vit.vit_s16,
    ...
    # ours
    "mvp": vit.vit_s16,
    "r3m": representations.get_r3m_encoder,
    "frankmocap_hand": representations.get_frankmocap_hand_encoder,
}
...
class Encoder(nn.Module):
    ...
    def __init__(self, model_name, pretrain_dir, freeze, emb_dim):
        ...
        pretrain_dir = os.getcwd().replace("mvp", "archive")
        print(colored("pretrained ckpt dir: "+pretrain_dir, "cyan"))
        ...
        # self.layer_norm = nn.LayerNorm(gap_dim)
    ...
    def forward(self, x):
        ...
        # img = img * torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1).to(img.device)
        # img = img + torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1).to(img.device)
        # import torchvision
        # torchvision.utils.save_image(img, "debug.png")
        # exit()

        if self.model_name in ['frankmocap_hand', 'r3m']:
            feat = self.backbone(x)
            return self.projector(self.layer_norm(feat)), feat
        else: # mvp class
    ...
    def forward_feat(self, feat):
        if self.model_name in ['frankmocap_hand', 'r3m']:
            return self.projector(self.layer_norm(feat))
        else: # mvp class
